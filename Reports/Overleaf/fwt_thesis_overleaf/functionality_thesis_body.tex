...
The repository is organized into a small number of clear layers that separate user interaction, persistent state, computational services, and downstream artifact generation. This separation is intentional. It allows the software to grow without collapsing into a collection of tightly coupled scripts.

\begin{figure}[htbp]
\centering
\maybegraphic{system_architecture_overview.png}
\caption{System architecture of the Flying Wing Tool.}
...
The central data contract is \texttt{core.state.Project}. Its role in the architecture is straightforward but fundamental: it is the mechanism by which geometry, analysis, mission assumptions, and derived results remain synchronized throughout the workflow. In other words, it is the state container that makes the integrated workflow possible. The object contains:
\begin{itemize}
...
\texttt{app/main\_window.py} provides the tabbed application shell, the project lifecycle actions (new/open/save), and the top-level entry point for twist optimization. In practice, it is the user's gateway into the shared project state and the integrated design workflow.
...
\texttt{app/tabs/geometry.py} is where the aircraft is primarily defined and inspected. It includes sub-tabs for:
\begin{itemize}
...
\texttt{app/tabs/analysis.py} provides airfoil sweep and contour-plotting workflows, including parameterized airfoil studies through \texttt{services/aero\_analysis.py}. This extends the tool beyond whole-aircraft analysis and gives the user a way to examine section-level aerodynamic behavior within the same environment.

\subsection{Mission Tab}
\texttt{app/tabs/mission.py} exposes the mission-analysis workflow and supports:
\begin{itemize}
\item sweep/batch mission operations,
\item mission profile setup and simulation execution,
\item phase statistics tables,
\item mission-oriented configuration workflows tied to propulsion and performance evaluation.
\end{itemize}

\subsection{Structure Tab}
\texttt{app/tabs/structure.py} contains the structural-analysis workflow and includes:
\begin{itemize}
...
\texttt{app/tabs/export.py} provides the outward-facing interoperability layer for the GUI and supports:
\begin{itemize}
\item Flow5 export,
...
\centering
\maybegraphic[\linewidth]{geometry_tab.png}
...
\begin{minipage}[t]{0.48\textwidth}
\centering
\maybegraphic[\linewidth]{analysis_tab.png}
...
\begin{minipage}[t]{0.48\textwidth}
\centering
\maybegraphic[\linewidth]{mission_tab.png}
...
\begin{minipage}[t]{0.48\textwidth}
\centering
\maybegraphic[\linewidth]{structures_tab.png}
...
\centering
\maybegraphic[\linewidth]{export_tab.png}

\small Export tab
\end{minipage}
\caption{Representative GUI views of the Flying Wing Tool.}
\label{fig:gui-tab-overview}
\end{figure}

\section{Core Analysis and Modeling Services}
\subsection{Geometry and Aerodynamics Service}
\texttt{services/geometry.py} (\texttt{AeroSandboxService}) is the central aero-geometry engine of the repository. More than any other module, it embodies the thesis goal of linking geometry definition to analysis results without forcing the user to rebuild the aircraft model between stages. Its major capabilities include:
\begin{itemize}
...
\texttt{services/aero\_analysis.py} provides section-level aerodynamic utilities that complement the whole-aircraft workflow. Its capabilities include:
\begin{itemize}
\item robust airfoil loading (NACA generation, named/profile files, DAT parsing),
\item NeuralFoil-based sweeps,
\item constrained airfoil-parameter studies built around thickness, smoothness, confidence, and weighted objective terms.
\end{itemize}

\subsection{Aero Model Abstractions}
\texttt{services/aero\_model.py} defines reusable aerodynamic model interfaces that allow mission and dynamics code to consume aerodynamic predictions in a consistent form. These include:
\begin{itemize}
...
\texttt{services/mission/} contains the logic required to move from a static aircraft definition to a time-dependent mission simulation. The subsystem includes:
\begin{itemize}
\item mission schema and phase typing (\texttt{mission\_definition.py}),
\item autopilot logic by phase (\texttt{autopilot.py}),
...
Pretrained propeller models are expected in \texttt{data/propeller\_models/*.pkl}. This arrangement makes it possible to mix analytical relations and learned surrogate behavior within the same propulsion workflow.

\subsection{Structural Subsystem}
\texttt{services/structure.py} is a substantial structural-analysis module that translates aerodynamic loading into preliminary structural consequences. It implements:
\begin{itemize}
...
\texttt{services/export/geometry\_builder.py} is the central geometry-construction module for CAD and export workflows. It supports building:
\begin{itemize}
\item skins,
\item spars,
...
The codebase supports generation and export in multiple formats:
\begin{itemize}
...
\begin{minipage}[t]{0.48\textwidth}
\centering
\maybegraphic[\linewidth]{cad_isometric.png}

\small Representative CAD/isometric geometry output
\end{minipage}
\hfill
\begin{minipage}[t]{0.48\textwidth}
\centering
...
\small Export workflow interface
\end{minipage}
\caption{Geometry and export views for interoperability workflows.}
\label{fig:export-interoperability-views}
\end{figure}

\section{Physical Methods Used}
This section summarizes the principal low-order physical relations embedded in the current repository. The equations shown here are not intended as generic textbook background. They are included because they correspond directly to the implemented models and therefore define the interpretation limits of the resulting outputs.
...
The aerodynamic implementation combines geometry-driven AeroSandbox analysis with lower-order fallback relations used in mission and robustness paths. At the most basic level, the force mapping follows the standard dynamic-pressure form:
\begin{align}
q &= \tfrac{1}{2}\rho V^2, \\
L &= q S C_L, \\
D &= q S C_D.
...
When a full aerodynamic model is unavailable, the fallback polar used in mission dynamics is:
\begin{align}
...
The implemented mission-oriented flight model uses the states
...
\end{align}
Wind is represented in ENU components and directly added to the air-relative velocity for kinematics.
...
In \texttt{services/mission/ground\_roll.py}, the along-runway one-degree-of-freedom model uses:
\begin{align}
...
The integrated propulsion chain is battery \(\rightarrow\) ESC \(\rightarrow\) motor \(\rightarrow\) propeller, with equilibrium solved at each time step. This matters because the mission solver is not driven by a single isolated thrust curve; it is driven by a coupled electrical and mechanical model.
...
The structural implementation combines beam theory, closed-section torsion, buckling checks, and post-buckling logic. The resulting model is explicitly preliminary in character, but it is still rich enough to expose the dominant trends and limiting mechanisms that matter during early design iteration.
...
The current aerostructural workflow is one-way by default: aerodynamic loads drive the structure, and the deformed structure is not iterated back into a fully coupled loop in the default path. That simplification is appropriate for the present scope, but it should be kept in mind when interpreting results as preliminary rather than fully aeroelastic.

\section{Data Inputs and Outputs}
\subsection{Inputs}
The repository is driven by a set of inputs that define the aircraft, the operating assumptions, and the analysis context. Typical user or model inputs include:
\begin{itemize}
...
\subsection{Outputs}
From those inputs, the software produces outputs that span the main preliminary-design disciplines. These outputs include:
\begin{itemize}
