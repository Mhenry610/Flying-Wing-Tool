...
Usage:
    from services.export.viewer_pv import open_pyvista_window
...
def _stack_meshes(meshes: List[pv.PolyData]) -> Optional[pv.PolyData]:
    ...
    if pv is None:
        return None
    # Filter out None and empty meshes (use n_points for compatibility with older PyVista)
    def _is_valid(m):
        if m is None:
            return False
        try:
            ...
            return False
    meshes = [m for m in meshes if _is_valid(m)]
    if not meshes:
        return None
    try:
...
def _build_cutter_wedge(processed, elevon_angle_deg: float) -> Optional[pv.PolyData]:
    """
    Build the deflection cutter wedge(s) aligned with the hinge spars (or fallback to rear spar).
    Iterates over all 'hingeSpar_' segments to generate a cutter for each control surface.
    """
    if pv is None:
        return None
    try:
        ...
        # 1. Identify Target Spars
        uids = list(processed.spar_uids or [])
        target_indices = []
        
        # Prefer explicit hinge spars
        for i, uid in enumerate(uids):
            if isinstance(uid, str) and uid.startswith("hingeSpar_"):
                target_indices.append(i)
                
        # Fallback to rearSpar_outboard if no hinge spars found but elevons exist
        if not target_indices and getattr(processed, "elevon_surfaces", None):
             try:
                idx = uids.index("rearSpar_outboard")
        ...
        for idx in target_indices:
            ...
            if use_top:
                a0, a1 = p_top_in, p_top_out
                h_in_vec = (p_bot_in - p_top_in) * height_scale
                h_out_vec = (p_bot_out - p_top_out) * height_scale
            elif use_bottom:
                a0, a1 = p_bot_in, p_bot_out
                h_in_vec = (p_top_in - p_bot_in) * height_scale
            ...
            # Normal Vector
            v_span = a1 - a0
            if _np.linalg.norm(v_span) < 1e-12: continue
            if _np.linalg.norm(h_in_vec) < 1e-12: continue
            ...
            if abs(forward_vec[0]) < 1e-9:
                if n[0] < 0: n = -n
            if use_center and n[0] < 0: n = -n

            # Build Mesh for this spar
            mesh = None
            if use_center:
                mid_in = 0.5 * (p_bot_in + p_top_in)
                mid_out = 0.5 * (p_bot_out + p_top_out)
                ...
                # Top wedge
                a0_top, a1_top = mid_in, mid_out
                h_in_top_vec = (p_top_in - mid_in) * height_scale
                h_out_top_vec = (p_top_out - mid_out) * height_scale
                b0_top = a0_top + h_in_top_vec + n * (_np.linalg.norm(h_in_top_vec) * tan_theta)
                b1_top = a1_top + h_out_top_vec + n * (_np.linalg.norm(h_out_top_vec) * tan_theta)
                
                # Bottom wedge
                a0_bot, a1_bot = mid_in, mid_out
                h_in_bot_vec = (p_bot_in - mid_in) * height_scale
                h_out_bot_vec = (p_bot_out - mid_out) * height_scale
                b0_bot = a0_bot + h_in_bot_vec + n * (_np.linalg.norm(h_in_bot_vec) * tan_theta)
                b1_bot = a1_bot + h_out_bot_vec + n * (_np.linalg.norm(h_out_bot_vec) * tan_theta)

                faces_all = [
                    [tuple(a0_top), tuple(a0_top + h_in_top_vec), tuple(b0_top)],
                    [tuple(a1_top), tuple(b1_top), tuple(a1_top + h_out_top_vec)],
                    [tuple(a0_top), tuple(a1_top), tuple(b1_top), tuple(b0_top)],
                    [tuple(a0_top + h_in_top_vec), tuple(b0_top), tuple(b1_top), tuple(a1_top + h_out_top_vec)],
                    [tuple(a0_bot), tuple(a0_bot + h_in_bot_vec), tuple(b0_bot)],
                    [tuple(a1_bot), tuple(b1_bot), tuple(a1_bot + h_out_bot_vec)],
                    [tuple(a0_bot), tuple(a1_bot), tuple(b1_bot), tuple(b0_bot)],
                    [tuple(a0_bot + h_in_bot_vec), tuple(b0_bot), tuple(b1_bot), tuple(a1_bot + h_out_bot_vec)],
                    [tuple(p_bot_in), tuple(p_top_in), tuple(p_top_out), tuple(p_bot_out)]
                ]
                mesh = _polyfaces_to_polydata(faces_all)
                ...
                mesh = _polyfaces_to_polydata([
                    [tuple(a0), tuple(a0 + h_in_vec), tuple(b0)],
                    [tuple(a1), tuple(b1), tuple(a1 + h_out_vec)],
                    [tuple(a0 + h_in_vec), tuple(b0), tuple(b1), tuple(a1 + h_out_vec)],
                    [tuple(a0), tuple(a1), tuple(b1), tuple(b0)],
                    [tuple(p_bot_in), tuple(p_top_in), tuple(p_top_out), tuple(p_bot_out)]
                ])
            
            if mesh: 
                print(f"[DEBUG] Built cutter mesh for spar index {idx}: {mesh}")
                meshes.append(mesh)
            else:
                print(f"[DEBUG] Failed to build mesh for spar index {idx}")

        print(f"[DEBUG] Total meshes built: {len(meshes)}")
        result = _stack_meshes(meshes)
        print(f"[DEBUG] Stacked mesh result: {result}")
        return result
    except Exception as e:
        print(f"[DEBUG] _build_cutter_wedge exception: {e}")
        import traceback
        traceback.print_exc()
        return None

def _deflect_elevon_generic(processed: ProcessedCpacs, angle_deg: float) -> Optional[List[List[List[Iterable[float]]]]]:
    """
    Deflect elevon surfaces using the pre-calculated hinge lines in ProcessedCpacs.
    Supports multiple independent control surfaces.
    """
    try:
        if processed is None or angle_deg == 0.0:
            return None
        ...
        # If we have explicit hinge lines, use them
        if hinge_lines and len(hinge_lines) == len(elevon_group):
            deflected_group = []
            for i, surface in enumerate(elevon_group):
                start, vec = hinge_lines[i]
                if start is None or vec is None:
                    deflected_group.append(surface)
                    continue
                
                # Rotate this surface about its specific hinge
                new_surface = []
                # surface is List[Face], Face is List[Point]
                for face in surface:
                     if not face: 
                         new_surface.append(face)
                         continue
                     rotated = _rotate_points_about_axis(face, start, vec, angle_deg)
                     new_surface.append(rotated)
                deflected_group.append(new_surface)
            return deflected_group
        
        # Fallback to single rear-spar hinge logic if no specific lines (Legacy / Single Elevon)
        # We can repurpose the 'bottom hinge' logic here or just return None to force fallback
        return _deflect_elevon_bottom_hinge_fallback(processed, angle_deg)

    except Exception:
        return None

def _deflect_elevon_bottom_hinge_fallback(processed: ProcessedCpacs, angle_deg: float) -> Optional[List[List[List[Iterable[float]]]]]:
    # (Original content of _deflect_elevon_bottom_hinge mostly, renamed for fallback)
    # ... Simplified for brevity, standard single-spar logic ...
    try:
        # Check for rearSpar_outboard
        uids = list(getattr(processed, "spar_uids", []) or [])
        try:
             idx = uids.index("rearSpar_outboard")
        except ValueError:
        ...
        spar_surfs = getattr(processed, "spar_surfaces", None)
        if not spar_surfs or idx >= len(spar_surfs): return None
        rear_faces = spar_surfs[idx]
        if not rear_faces or len(rear_faces) < 2: return None
        rear_face = rear_faces[1]
        ...
        for surface in processed.elevon_surfaces:
            new_s = []
            for face in surface:
                if not face: new_s.append(face); continue
                new_s.append(_rotate_points_about_axis(face, p0, p1-p0, angle_deg))
            deflected_group.append(new_s)
        return deflected_group
    except Exception:
        return None
...
def _build_wing_mesh(wing_quads: List[List[Iterable[float]]],
    ...
    if dihedraled_rib_profiles is not None and len(dihedraled_rib_profiles) >= 2:
